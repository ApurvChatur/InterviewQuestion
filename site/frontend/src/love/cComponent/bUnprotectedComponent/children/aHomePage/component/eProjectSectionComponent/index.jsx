        ...
        <div className="container px-5 py-24 mx-auto flex flex-wrap flex-col">
          ...
          <div className="flex mx-auto mb-10 justify-center">
            <ScrollArea className="w-80 lg:w-2/3 whitespace-nowrap rounded-md">
              {Redux.state.ReceivedObject?.Retrieve?.ProjectSectionRetrieve?.projectGroups?.map((each, index) => (
                <a 
                  ...
                  ${activeProjectGroup === each.title ? 
                    "bg-[#9000FF] dark:bg-[#FFE8F5] text-[#FFE8F5] dark:text-[#9000FF] border-[#FFE8F5] dark:border-[#9000FF] rounded-t" : 
              ...
              <ScrollBar orientation="horizontal" className="bg-[#9000FF] dark:bg-[#FFE8F5]" />
            </ScrollArea>
          </div>
          
          {Redux.state.ReceivedObject?.Retrieve?.ProjectSectionRetrieve?.projectGroups?.map((each, index) => (
            <React.Fragment key={index}>
              <div className={`flex flex-wrap text-center bg-[#9000FF] dark:bg-[#FFE8F5] text-[#FFE8F5] dark:text-[#9000FF] bg-opacity-60 px-4 py-6 rounded-lg ${activeProjectGroup === each.title ? "" : "hidden"}`}>
                ...
                {each?.projects?.map((each1, index1) => (
                  <React.Fragment key={index1}>
                    <div className="p-4 w-full md:w-1/3 text-left">
                      <div className="h-full bg-[#FFE8F5] dark:bg-[#9000FF] text-[#9000FF] dark:text-[#FFE8F5] rounded-lg overflow-hidden">
                          ...
                          <div className="flex items-center flex-wrap ">
                            ...
                            {each1?.links?.map((each2, index2) => (
                              each2?.title === "Visit Application" ? (
                                <span key={index2} className="mr-3 inline-flex items-center lg:ml-auto md:ml-0 ml-auto leading-none text-sm py-1 border-r-2 border-[#9000FF] dark:border-[#FFE8F5]">
                              ...
                              each2?.title === "Visit Admin" ? (
                                <span key={index2} className="mr-3 inline-flex items-center leading-none text-sm py-1 border-r-2 border-[#9000FF] dark:border-[#FFE8F5]">
                                  <Link to={each2?.url} target="_blank" rel="noreferrer" >
